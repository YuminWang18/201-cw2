...
# Settings
py_files = ['main.py', 'u_distance.json','v_distance.json','w_distance.json','x_distance.json'
    ,'u_ip.json','v_ip.json','w_ip.json','x_ip.json']
...
def run():
    ...
    try:
        ssh.connect(remote_ip, username=remote_username, password=remote_password, port=22, timeout=15)
        ...
        if len(py_files) > 0:
            ...
            stdin, stdout, stderr = ssh.exec_command(f'cd {remote_current_working_directory}; {remote_python_interpreter} {main_py} --node u', bufsize=1, get_pty=True)
